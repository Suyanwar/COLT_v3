...
if($data){
	...
	foreach($data as $list){
		...
		$engagement_ratio = ($this->report_instagram->users($list->account_id, 'MostActiveCount', $date) / ($list->fans ? $list->fans : 1)) * 100;
		...
		echo ($list->account_id == $account->account_id) ? '<tr class="active" style="text-align:right">' : '<tr style="text-align:right">';
		echo '<td>'.$i.'</td>';
