...
$('#growth_MonthlyGrowth').highcharts({
	...
	series: [{
		...
		for($i=$start; $i <= $end; $i++){
			for($m=1; $m <= count($arr); $m++){
				$current = ($i.$m == $cur_month) ? 0 : $this->report_instagram->growth($account, 'MonthlyFans', $i.'-'.(($m > 9) ? $m : '0'.$m));
				echo $x ? ',' : '';
				if($x){
					echo $current ? $current - $last : 0;
				}
				else echo $current - $this->report_instagram->growth($account, 'DailyPrevious', $i.'-01-01');
