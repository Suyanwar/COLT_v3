...
<div id="chart_DailyPost"></div>
<!--br />
<div id="chart_DailyPost2"></div-->
<script type="text/javascript">
$('#chart_DailyPost').highcharts({
	chart: {
		type: 'heatmap',
		marginTop: 40,
		marginBottom: 80
	...
	xAxis: {
		...
		<?php
		for($i=0; $i < 24; $i++)
		$sph[$i] = $this->report_instagram->primetime($account, 'SumPost', array($date, '%H', $i));
	...
	yAxis: {
		...
		$spw[$ii] = $this->report_instagram->primetime($account, 'SumPost', array($date, '%w', $ii));
	...
	series: [{
		...
		for($i=0; $i < 24; $i++){
			for($ii=0; $ii < 7; $ii++){
				$pt[$i.'_'.$ii] = $this->report_instagram->primetime($account, 'DailyPost', array($date, (($i > 9) ? $i : '0'.$i).'/'.$ii));
				echo $x ? ',' : '';
				echo '['.$i.', '.$ii.', '.$pt[$i.'_'.$ii].']';
				$x=1;
			}
