...
$('#chart_MonthlyGrowth').highcharts({
	...
	foreach($period as $year){
		...
		for($i=1; $i <= count($months); $i++){
			...
			$counts = $this->report_instagram->growth($account, 'MonthlyFans', $year.'-'.(($i > 9) ? $i : '0'.$i));
			if($s){
				$count = $last ? ($counts - $last) : 0;
			}
			else $count = $counts - $this->report_instagram->growth($account, 'MonthlyFans', date('Y-m', strtotime('-1 month', strtotime($year.'-'.(($i > 9) ? $i : '0'.$i).'-01'))));
