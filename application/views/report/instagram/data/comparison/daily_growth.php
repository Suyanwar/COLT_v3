...
$('#chart_DailyGrowth').highcharts({
	...
	foreach($period as $month){
		...
		for($i=1; $i < 32; $i++){
			...
			$counts = $this->report_instagram->growth($account, 'CountFans', array('%Y-%m-%d', $month.'-'.(($i > 9) ? $i : '0'.$i)));
			if($s){
				$count = $last ? ($counts - $last) : 0;
			}
			else $count = $counts - $this->report_instagram->growth($account, 'CountFans', array('%Y-%m-%d', date('Y-m-d', strtotime('-1 days', strtotime($month.'-'.(($i > 9) ? $i : '0'.$i))))));
