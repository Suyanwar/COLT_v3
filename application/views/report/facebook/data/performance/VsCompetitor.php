...
$('#performance_VsCompetitor').highcharts({
	...
	for($i=2; $i >= 0; $i--){
		echo $this->report_facebook->performance($account->account_id, 'VsCompetitor', date('Y-m', strtotime("- $i month", $date)));
		echo $i ? ',' : '';
	}
	echo ']}';
	...
	foreach($competitor as $list){
		...
		for($i=2; $i >= 0; $i--){
			echo $this->report_facebook->performance($list->account_id, 'VsCompetitor', date('Y-m', strtotime("- $i month", $date)));
			echo $i ? ',' : '';
		}
		echo ']}';
	}
