...
<div id="meta">
	<a id="acc">
        <img src="<?php echo base_url('static/i/frame-twitter.png') ?>" style="background-image:url(<?php echo $account->photo_profile ?>)">
        <?php echo $account->name.'<sup><img src="'.base_url('static/i/flag-'.$account->fl_active.'.png').'" title="'.($account->fl_active ? 'Active' : 'Idle').'"></sup>' ?>
	</a>
    <a href="<?php echo $account->username ? 'https://twitter.com/'.$account->username : 'https://twitter.com/intent/user?user_id='.$account->socmed_id ?>" target="_blank"><sup class="fa fa-external-link"></sup></a>
</div>
...
<ul class="ca-menu-circle">
    ...
    foreach($menus as $mn){ ?>
    <li>
        <a onclick="report_content('twitter', '<?php echo $mn[1] ?>', <?php echo $account->account_id ?>)">
