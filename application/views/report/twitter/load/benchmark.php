...
<div class="content">
	...
	foreach($data as $list){
		echo '<h1><span class="fa fa-circle-o-notch"></span> '.$list->category.' <sup><span class="fa fa-info-circle tooltip"><span class="tooltiptext"><b>Category '.$list->category.'</b><hr>Data <i>benchmark</i> akun twitter terdaftar yang di dapatkan berdasarkan kategori <i>'.$list->category.'</i>.</span></span></sup></h1>';
...
function reportLoad(){
	...
	if($data):
	foreach($data as $list){
		echo 'report_load("benchmark_'.str_replacing('alpha_num', $list->category).'", "twitter", "benchmark", '.$account.', {"category":"'.$list->category.'"});';
	}
	endif;
	?>
}
