...
$('#chart_DailyPost').highcharts({
	...
	xAxis: {
		...
		<?php
		for($i=0; $i < 24; $i++)
		$sph[$i] = $this->report_twitter->primetime($account, 'SumPost', array($date, '%H', $i));
	...
	yAxis: {
		...
		$spw[$ii] = $this->report_twitter->primetime($account, 'SumPost', array($date, '%w', $ii));
	...
	series: [{
		...
		for($i=0; $i < 24; $i++){
			for($ii=0; $ii < 7; $ii++){
				$pt[$i.'_'.$ii] = $this->report_twitter->primetime($account, 'DailyPost', array($date, (($i > 9) ? $i : '0'.$i).'/'.$ii));
				echo $x ? ',' : '';
				echo '['.$i.', '.$ii.', '.$pt[$i.'_'.$ii].']';
				$x=1;
			}
