...
<div id="chart_DailyShare"></div>
<!--br />
<div id="chart_DailyShare2"></div-->
<script type="text/javascript">
$('#chart_DailyShare').highcharts({
	chart: {
		type: 'heatmap',
		marginTop: 40,
		marginBottom: 80
	...
	xAxis: {
		...
		categories: [
		<?php
		$feeds = $this->report_twitter->primetime($account, 'FeedIds', $date);
		for($i=0; $i < 24; $i++)
		$sph[$i] = $this->report_twitter->primetime($account, 'SumSharex', array($feeds, '%H', (($i > 9) ? $i : '0'.$i)));
	...
	yAxis: {
		...
		$spw[$ii] = $this->report_twitter->primetime($account, 'SumSharex', array($feeds, '%w', $ii));
	...
	series: [{
		...
		for($i=0; $i < 24; $i++){
			for($ii=0; $ii < 7; $ii++){
				$pt[$i.'_'.$ii] = $this->report_twitter->primetime($account, 'DailySharex', array($feeds, (($i > 9) ? $i : '0'.$i).'/'.$ii));
				echo $x ? ',' : '';
				echo '['.$i.', '.$ii.', '.$pt[$i.'_'.$ii].']';
				$x=1;
			}
