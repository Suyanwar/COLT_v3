...
$('#chart_DailyShare').highcharts({
	...
	xAxis: {
		...
		<?php
		for($i=0; $i < 24; $i++)
		$sph[$i] = $this->report_twitter->primetime($account, 'SumShare', array($date, '%H', $i));
	...
	yAxis: {
		...
		$spw[$ii] = $this->report_twitter->primetime($account, 'SumShare', array($date, '%w', $ii));
	...
	series: [{
		...
		for($i=0; $i < 24; $i++){
			for($ii=0; $ii < 7; $ii++){
				$pt[$i.'_'.$ii] = $this->report_twitter->primetime($account, 'DailyShare', array($date, (($i > 9) ? $i : '0'.$i).'/'.$ii));
				echo $x ? ',' : '';
				echo '['.$i.', '.$ii.', '.$pt[$i.'_'.$ii].']';
				$x=1;
			}
