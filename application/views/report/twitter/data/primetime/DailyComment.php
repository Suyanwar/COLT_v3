...
$('#chart_DailyComment').highcharts({
	...
	xAxis: {
		...
		for($i=0; $i < 24; $i++)
		$sph[$i] = $this->report_twitter->primetime($account, 'SumCommentx', array($feeds, '%H', (($i > 9) ? $i : '0'.$i)));
	...
	yAxis: {
		...
		$spw[$ii] = $this->report_twitter->primetime($account, 'SumCommentx', array($feeds, '%w', $ii));
	...
	series: [{
		...
		for($i=0; $i < 24; $i++){
			for($ii=0; $ii < 7; $ii++){
				$pt[$i.'_'.$ii] = $this->report_twitter->primetime($account, 'DailyCommentx', array($feeds, (($i > 9) ? $i : '0'.$i).'/'.$ii));
				echo $x ? ',' : '';
				echo '['.$i.', '.$ii.', '.$pt[$i.'_'.$ii].']';
				$x=1;
			}
