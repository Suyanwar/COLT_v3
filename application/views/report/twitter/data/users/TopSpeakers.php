...
foreach($data as $list){
	echo '<li><table><tr><td><a href="'.($list->u ? 'https://twitter.com/'.$list->u : 'https://twitter.com/intent/user?user_id='.$list->s).'" target="_blank"><img src="'.base_url('static/i/spacer.gif').'" style="background-image:url('.$list->p.')"></a></td><td>Amplification: <b>'.number_format($list->i).'</b><br>Activity: <b>'.number_format($list->t).'</b><br>Followers: <b>'.(($list->f > 1) ? number_format($list->f) : '').'</b><br>Loc: <b>'.$list->l.'</b></td></tr><tr><td colspan="2"><div><a href="'.($list->u ? 'https://twitter.com/'.$list->u : 'https://twitter.com/intent/user?user_id='.$list->s).'" target="_blank">'.($list->u ? '@'.$list->u : $list->n).'</a></div></td></tr></table></li>';
}
echo '</ul>';
