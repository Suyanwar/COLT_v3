...
class Report_facebook extends CI_Model {
	...
	function impression($feeds, $date)
	{
		...
		$comment = 0;
		$Qcomment = $this->db->select("IFNULL(b.count, ".$this->assuming_friend().") AS total")->from('comment a')->join("(SELECT count, user_id FROM user_follower WHERE created_time <= '$period[1]' GROUP BY user_id ORDER BY created_time DESC) b", 'a.user_id=b.user_id', 'left')->where("a.feed_id IN($feeds)", NULL)->get();
		...
		$likes = 0;
		$Qlikes = $this->db->select("IFNULL(b.count, ".$this->assuming_friend().") AS total")->from('likes a')->join("(SELECT count, user_id FROM user_follower WHERE created_time <= '$period[1]' GROUP BY user_id ORDER BY created_time DESC) b", 'a.user_id=b.user_id', 'left')->where("a.feed_id IN($feeds)", NULL)->get();
		if($Qlikes->num_rows()){
			foreach($Qlikes->result() as $list){
				connection_abort();
				$likes = $list->total + $likes;
			}
		}
		
		$share = 0;
		$Qshare = $this->db->select("IFNULL(b.count, ".$this->assuming_friend().") AS total")->from('share a')->join("(SELECT count, user_id FROM user_follower WHERE created_time <= '$period[1]' GROUP BY user_id ORDER BY created_time DESC) b", 'a.user_id=b.user_id', 'left')->where("a.feed_id IN($feeds)", NULL)->get();
		if($Qshare->num_rows()){
	...
	{
		$app = $this->db->from('app')->where(array('fl_active' => 1, 'socmed' => 'facebook'))->order_by('RAND()')->limit(1)->get();
		if($app->num_rows()){
		...
		switch($type){
			case 'MostCommenters':
				...
				if(file_exists($rawdata)){
					...
					return NULL;
					/*$feeds = $this->posts($account_id, $date);
					$Qlist = $this->db->select("COUNT(1) AS t, b.socmed_id AS s, b.username AS u, b.name AS n, b.photo_profile AS p, IFNULL(b.city, b.location) AS l, c.count AS f")->from('comment a')->join('user b', 'a.user_id=b.user_id')->join("(SELECT count, user_id FROM user_follower WHERE created_time <= '".date('Y-m-d', $period[1])."' GROUP BY user_id ORDER BY created_time DESC) c", 'a.user_id=c.user_id', 'left')->where(array("a.feed_id IN($feeds)" => NULL))->group_by('a.user_id')->order_by('t', 'DESC')->limit(10)->get();
					//, "a.user_id NOT IN('FB-1130343030310068', 'FB-513307648855711', 'FB-10208575912185060')" => NULL
					if($Qlist->num_rows()){
						return $Qlist->result();
					}
					else return NULL;*/
			...
			case 'MostLikers':
				...
				if(file_exists($rawdata)){
					...
					return NULL;
					/*$feeds = $this->posts($account_id, $date);
					$Qlist = $this->db->select("COUNT(1) AS t, b.socmed_id AS s, b.username AS u, b.name AS n, b.photo_profile AS p, IFNULL(b.city, b.location) AS l, c.count AS f")->from('likes a')->join('user b', 'a.user_id=b.user_id')->join("(SELECT count, user_id FROM user_follower WHERE created_time <= '".date('Y-m-d', $period[1])."' GROUP BY user_id ORDER BY created_time DESC) c", 'a.user_id=c.user_id', 'left')->where(array("a.feed_id IN($feeds)" => NULL))->group_by('a.user_id')->order_by('t', 'DESC')->limit(10)->get();
					//, "a.user_id NOT IN('FB-1130343030310068', 'FB-513307648855711', 'FB-10208575912185060')" => NULL
					if($Qlist->num_rows()){
						return $Qlist->result();
					}
			...
			case 'MostActive':
				...
				if(file_exists($rawdata)){
					...
					/*$feeds = $this->posts($account_id, $date);
					$Qlist = $this->db->query("
						SELECT SUM(x.`activity`) AS `t`, x.`user_id`, b.`socmed_id` AS `s`, b.`username` AS `u`, b.`name` AS `n`, b.`photo_profile` AS `p`, IFNULL(b.`city`, b.`location`) AS `l`, c.`count` AS `f` FROM (
						...
						LEFT JOIN (SELECT `count`, `user_id` FROM `user_follower` WHERE `created_time` <= '".date('Y-m-d', $period[1])."' GROUP BY `user_id` ORDER BY `created_time` DESC) c ON x.`user_id`=c.`user_id`
						GROUP BY b.`socmed_id`
						ORDER BY `t` DESC LIMIT 10
					");
					//WHERE b.`socmed_id` NOT IN('1130343030310068', '513307648855711', '10208575912185060')
					if($Qlist->num_rows()){
						return $Qlist->result();
					}
					else return NULL;*/
			...
			case 'TopSpeakers':
				...
				if(file_exists($rawdata)){
					...
					/*$feeds = $this->posts($account_id, $date);
					$Qlist = $this->db->query("
						SELECT SUM(x.`activity`) AS `t`, (SUM(x.`activity`) * (CASE IFNULL(c.`count`, 0) WHEN 0 THEN 1 ELSE IFNULL(c.`count`, 0) END)) AS `i`, x.`user_id`, b.`socmed_id` AS `s`, b.`username` AS `u`, b.`name` AS `n`, b.`photo_profile` AS `p`, IFNULL(b.`city`, b.`location`) AS `l`, c.`count` AS `f` FROM (
							SELECT COUNT(1) AS `activity`, `user_id` FROM `comment` WHERE `feed_id` IN($feeds) GROUP BY `user_id`
							UNION ALL
							SELECT COUNT(1) AS `activity`, `user_id` FROM `likes` WHERE `feed_id` IN($feeds) GROUP BY `user_id`
							UNION ALL
	...
	public function demography($account_id, $type, $date)
	{
		...
		switch($type){
			case 'Country':
				$Qlist = $this->db->select('COUNT(1) AS total, country AS title')->from('user')->where(array("socmed_id IN($users_id)" => NULL, 'country IS NOT NULL' => NULL, 'socmed' => 'facebook'))->group_by('country')->order_by('total', 'DESC')->get();
				if($Qlist->num_rows()){
					return $Qlist->result();
				}
				else return NULL;
			...
			case 'State':
				$Qlist = $this->db->select('COUNT(1) AS total, state AS title')->from('user')->where(array("socmed_id IN($users_id)" => NULL, 'state IS NOT NULL' => NULL, 'socmed' => 'facebook'))->group_by('state')->order_by('total', 'DESC')->get();
				if($Qlist->num_rows()){
					return $Qlist->result();
				}
				else return NULL;
			...
			case 'City':
				$Qlist = $this->db->select('COUNT(1) AS total, city AS title')->from('user')->where(array("socmed_id IN($users_id)" => NULL, 'city IS NOT NULL' => NULL, 'socmed' => 'facebook'))->group_by('city')->order_by('total', 'DESC')->get();
				if($Qlist->num_rows()){
					return $Qlist->result();
				}
				else return NULL;
