...
function set_userdata($name, $value=NULL)
{
	...
	else $_SESSION[USERDATA_KEY][$name] = $value;
}
...
function unset_userdata($name)
{
	...
	if($count == 1){
		...
		if(isset($_SESSION[USERDATA_KEY][$name])){
			unset($_SESSION[USERDATA_KEY][$name]);
		}elseif(isset($_SESSION[USERDATA_KEY][$name[$i]])){
...
function set_flashdata($name, $value=NULL)
{
	...
	else $_SESSION[FLASHDATA_KEY][$name] = $value;
}
